...
#include "GASSGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class UGASSRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class EGASSAbilityTargetingSource : uint8
{
...
/**
 * UGASSGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UGASSGameplayAbility_RangedWeapon : public UGASSGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UGASSGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="GASS|Ability")
	UGASSRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UGASSRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EGASSAbilityTargetingSource Source) const;
