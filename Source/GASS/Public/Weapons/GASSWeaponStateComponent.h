...
#include "GASSWeaponStateComponent.generated.h"
...
struct FGASSScreenSpaceHitLocation
{
...
struct FGASSServerSideHitMarkerBatch
{
	FGASSServerSideHitMarkerBatch() { }

	FGASSServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FGASSScreenSpaceHitLocation> Markers;
...
UCLASS()
class UGASSWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UGASSWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FGASSScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FGASSScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FGASSServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
