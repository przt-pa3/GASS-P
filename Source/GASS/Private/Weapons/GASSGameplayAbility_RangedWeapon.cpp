...
#include "Weapons/GASSGameplayAbility_RangedWeapon.h"
#include "Weapons/GASSRangedWeaponInstance.h"
#include "Physics/GASSCollisionChannels.h"
#include "GASS/GASSLogChannels.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Weapons/GASSWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/GASSGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GASSGameplayAbility_RangedWeapon)

namespace GASSConsoleVariables
{
	static float DrawBulletTracesDuration = 0.0f;
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("GASS.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("GASS.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("GASS.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UGASSGameplayAbility_RangedWeapon::UGASSGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UGASSRangedWeaponInstance* UGASSGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UGASSRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UGASSGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogGASSAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UGASSRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UGASSGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UGASSGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UGASSGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return GASS_TraceChannel_Weapon;
}

FHitResult UGASSGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UGASSGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UGASSGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EGASSAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UGASSWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UGASSWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EGASSAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EGASSAbilityTargetingSource::CameraTowardsFocus) || (Source == EGASSAbilityTargetingSource::PawnTowardsFocus) || (Source == EGASSAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EGASSAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EGASSAbilityTargetingSource::WeaponForward) || (Source == EGASSAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EGASSAbilityTargetingSource::PawnTowardsFocus) || (Source == EGASSAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UGASSGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (GASSConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, GASSConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UGASSGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UGASSRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EGASSAbilityTargetingSource::CameraTowardsFocus);
		...
		if (GASSConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, GASSConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UGASSGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UGASSRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (GASSConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, GASSConsoleVariables::DrawBulletHitRadius, FColor::Red, false, GASSConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UGASSGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UGASSGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UGASSGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UGASSWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UGASSWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogGASSAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UGASSGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UGASSWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UGASSWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FGASSGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FGASSGameplayAbilityTargetData_SingleTargetHit();
