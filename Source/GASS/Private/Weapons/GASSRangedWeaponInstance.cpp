...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GASSRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_GASS_Weapon_SteadyAimingCamera, "GASS.Weapon.SteadyAimingCamera");

UGASSRangedWeaponInstance::UGASSRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UGASSRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UGASSRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UGASSRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UGASSRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UGASSRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UGASSRangedWeaponInstance::OnArmed()
{
	Super::OnArmed();
}

void UGASSRangedWeaponInstance::OnDisarmed()
{
	Super::OnDisarmed();
}

void UGASSRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UGASSRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UGASSRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UGASSRangedWeaponInstance::AddSpread()
{
...
float UGASSRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UGASSRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UGASSRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UGASSRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UGASSCameraComponent* CameraComponent = UGASSCameraComponent::FindCameraComponent(Pawn))
	{
		float TopCameraWeight;
		FGameplayTag TopCameraTag;
		CameraComponent->GetBlendInfo(/*out*/ TopCameraWeight, /*out*/ TopCameraTag);

		AimingAlpha = (TopCameraTag == TAG_GASS_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
