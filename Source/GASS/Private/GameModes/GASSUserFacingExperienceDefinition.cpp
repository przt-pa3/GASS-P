...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GASSUserFacingExperienceDefinition)

namespace GASS::Experience
{
	UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Platform_Trait_ReplaySupport, "Platform.Trait.ReplaySupport");
};

UCommonSession_HostSessionRequest* UGASSUserFacingExperienceDefinition::CreateHostingRequest() const
{
	...
	if (ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(GASS::Experience::TAG_Platform_Trait_ReplaySupport.GetTag()))
	{
		if (bRecordReplay)
		{
