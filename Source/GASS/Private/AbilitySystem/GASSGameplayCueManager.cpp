...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GASSGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EGASSEditorLoadMode
{
...
namespace GASSGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("GASS.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via GASSGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UGASSGameplayCueManager::DumpGameplayCues));

	static EGASSEditorLoadMode LoadMode = EGASSEditorLoadMode::LoadUpfront;
}
...
UGASSGameplayCueManager::UGASSGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UGASSGameplayCueManager* UGASSGameplayCueManager::Get()
{
	return Cast<UGASSGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UGASSGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UGASSGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogGASS, Warning, TEXT("UGASSGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UGASSGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (GASSGameplayCueManagerCvars::LoadMode)
	{
	case EGASSEditorLoadMode::LoadUpfront:
		return true;
	case EGASSEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EGASSEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UGASSGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UGASSGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UGASSGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UGASSGameplayCueManager* GCM = Cast<UGASSGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogGASS, Error, TEXT("DumpGameplayCues failed. No UGASSGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogGASS, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogGASS, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogGASS, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogGASS, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogGASS, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogGASS, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogGASS, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogGASS, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogGASS, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogGASS, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogGASS, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogGASS, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UGASSGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UGASSGameplayCueManager* StrongThis = Get())
					{
...
void UGASSGameplayCueManager::HandlePostGarbageCollect()
{
...
void UGASSGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogGASS, Warning, TEXT("UGASSGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UGASSGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (GASSGameplayCueManagerCvars::LoadMode)
	{
	case EGASSEditorLoadMode::LoadUpfront:
...
void UGASSGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UGASSGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UGASSGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UGASSGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UGASSGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UGASSGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
