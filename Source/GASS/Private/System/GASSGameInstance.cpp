...
#include "System/GASSGameInstance.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GASSGameInstance)

namespace GASS
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarGASSTestEncryption(
		TEXT("GASS.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarGASSUseDTLSEncryption(
		TEXT("GASS.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using GASS.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarGASSTestDTLSFingerprint(
		TEXT("GASS.TestDTLSFingerprint"),
...
UGASSGameInstance::UGASSGameInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UGASSGameInstance::Init()
{
	Super::Init();
	...
	{
		ComponentManager->RegisterInitState(GASSGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(GASSGameplayTags::InitState_DataAvailable, false, GASSGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(GASSGameplayTags::InitState_DataInitialized, false, GASSGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(GASSGameplayTags::InitState_GameplayReady, false, GASSGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UGASSGameInstance::OnPreClientTravelToSession);
	}
}

void UGASSGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
AGASSPlayerController* UGASSGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AGASSPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UGASSGameInstance::CanJoinRequestedSession() const
{
...
void UGASSGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (GASS::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (GASS::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("GASSTest.pem");
...
void UGASSGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UGASSGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (GASS::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (GASS::bUseDTLSEncryption)
		{
