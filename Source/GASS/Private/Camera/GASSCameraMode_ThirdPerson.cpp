...
#include "Camera/GASSCameraMode_ThirdPerson.h"
#include "Camera/GASSCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/GASSPenetrationAvoidanceFeeler.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GASSCameraMode_ThirdPerson)

namespace GASSCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UGASSCameraMode_ThirdPerson::UGASSCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FGASSPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FGASSPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FGASSPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FGASSPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FGASSPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FGASSPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FGASSPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UGASSCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UGASSCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UGASSCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UGASSCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IGASSCameraAssistInterface* TargetControllerAssist = Cast<IGASSCameraAssistInterface>(TargetController);

	IGASSCameraAssistInterface* TargetActorAssist = Cast<IGASSCameraAssistInterface>(TargetActor);
	...
	IGASSCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IGASSCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IGASSCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IGASSCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UGASSCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IGASSCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FGASSPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(GASSCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UGASSCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UGASSCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
